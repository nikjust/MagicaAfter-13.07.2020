...
import net.mcreator.magica.block.HeartiumGrassBlock;
...
public class HeartiBiome extends MagicaModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(HeartiumGrassBlock.block.getDefaultState(),
							HeartiumBlock.block.getDefaultState(), HeartiumBlock.block.getDefaultState())));
			setRegistryName("hearti");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == HeartiumGrassBlock.block.getDefaultState().getBlock()
							|| ground == HeartiumBlock.block.getDefaultState().getBlock())
							&& (ground2 == HeartiumGrassBlock.block.getDefaultState().getBlock()
									|| ground2 == HeartiumBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), HeartiumBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.OAK_LOG.getDefaultState().getBlock()
					...
					|| blockType == HeartiumGrassBlock.block.getDefaultState().getBlock()
					|| blockType == HeartiumBlock.block.getDefaultState().getBlock();
		}
