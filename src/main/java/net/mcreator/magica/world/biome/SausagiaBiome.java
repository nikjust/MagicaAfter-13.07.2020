...
public class SausagiaBiome extends MagicaModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-26164).waterFogColor(-26164)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(SausageBlock.block.getDefaultState(),
							SausageBlock.block.getDefaultState(), SausageBlock.block.getDefaultState())));
			setRegistryName("sausagia");
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == SausageBlock.block.getDefaultState().getBlock() || ground == SausageBlock.block.getDefaultState().getBlock())
							&& (ground2 == SausageBlock.block.getDefaultState().getBlock()
									|| ground2 == SausageBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), SausageBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == SausageBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, SausageBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, SausageBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == SausageBlock.block.getDefaultState().getBlock()
					|| blockType == SausageBlock.block.getDefaultState().getBlock() || blockType == SausageBlock.block.getDefaultState().getBlock()
					|| blockType == SausageBlock.block.getDefaultState().getBlock();
		}
