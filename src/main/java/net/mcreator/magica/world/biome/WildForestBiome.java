...
import net.mcreator.magica.block.WildForestLightGreenVineBlock;
...
public class WildForestBiome extends MagicaModElements.ModElement {
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == WildForestLightGreenVineBlock.block.getDefaultState().getBlock()
												|| state.getBlock() == WildForestGreenVineBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, WildForestGreenVineBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, WildForestGreenVineBlock.block.getDefaultState(), bbox);
								...
								if (genh > 0) {
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(-1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(-1, genh, 0),
												WildForestLightGreenVineBlock.block.getDefaultState(), bbox);
									if (rand.nextInt(3) > 0 && world.isAirBlock(position.add(1, genh, 0)))
										setTreeBlockState(changedBlocks, world, position.add(1, genh, 0),
												WildForestLightGreenVineBlock.block.getDefaultState(), bbox);
		...
		private void addVines(IWorld world, BlockPos pos, Set<BlockPos> changedBlocks, MutableBoundingBox bbox) {
			setTreeBlockState(changedBlocks, world, pos, WildForestLightGreenVineBlock.block.getDefaultState(), bbox);
			int i = 5;
			for (BlockPos blockpos = pos.down(); world.isAirBlock(blockpos) && i > 0; --i) {
				setTreeBlockState(changedBlocks, world, blockpos, WildForestLightGreenVineBlock.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR
					|| blockType == WildForestGreenVineBlock.block.getDefaultState().getBlock()
					|| blockType == WildForestGreenVineBlock.block.getDefaultState().getBlock()
