...
public class KillingVoidBiome extends MagicaModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.5f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					...
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(KillingVoidStoneBlock.block.getDefaultState(),
							KillingVoidStoneBlock.block.getDefaultState(), KillingVoidStoneBlock.block.getDefaultState())));
			setRegistryName("killing_void");
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(MysteryCreeperEntity.entity, 20, 4, 4));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == KillingVoidStoneBlock.block.getDefaultState().getBlock()
							|| ground == KillingVoidStoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == KillingVoidStoneBlock.block.getDefaultState().getBlock()
									|| ground2 == KillingVoidStoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), KillingVoidStoneBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												KillingVoidStoneBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == Blocks.DARK_OAK_LOG.getDefaultState().getBlock()
					|| blockType == Blocks.AIR.getDefaultState().getBlock() || blockType == KillingVoidStoneBlock.block.getDefaultState().getBlock()
					|| blockType == KillingVoidStoneBlock.block.getDefaultState().getBlock();
		}
