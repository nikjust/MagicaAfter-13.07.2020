...
package net.mcreator.magica.world.dimension;
...
import net.mcreator.magica.procedures.PainDimensionPlayerLeavesDimensionProcedure;
import net.mcreator.magica.procedures.PainDimensionPlayerEntersDimensionProcedure;
import net.mcreator.magica.item.PainDimensionItem;
import net.mcreator.magica.block.CorruptionBlock;
import net.mcreator.magica.block.BurnedStoneBlock;
...
public class PainDimensionDimension extends MagicaModElements.ModElement {
	@ObjectHolder("magica:pain_dimension")
	public static final ModDimension dimension = null;
	@ObjectHolder("magica:pain_dimension_portal")
	public static final CustomPortalBlock portal = null;
	public static DimensionType type = null;
	private static Biome[] dimensionBiomes;
	public PainDimensionDimension(MagicaModElements instance) {
		super(instance, 162);
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("pain_dimension"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("magica:pain_dimension")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("magica:pain_dimension"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("magica:pain_dimension"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("magica:pain")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new PainDimensionItem().setRegistryName("pain_dimension"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("pain_dimension_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == CorruptionBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == CorruptionBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == CorruptionBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag ? CorruptionBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, CorruptionBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		if (event.getFrom() == type) {
			{
				...
				PainDimensionPlayerLeavesDimensionProcedure.executeProcedure($_dependencies);
			}
		}
		if (event.getTo() == type) {
			{
				Map<String, Object> $_dependencies = new HashMap<>();
				$_dependencies.put("entity", entity);
				PainDimensionPlayerEntersDimensionProcedure.executeProcedure($_dependencies);
			}
		}
	}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new OverworldGenSettings() {
				public BlockState getDefaultBlock() {
					return BurnedStoneBlock.block.getDefaultState();
				}
			...
			if (!biomesPatched) {
				for (Biome biome : this.biomes) {
					biome.addCarver(GenerationStage.Carving.AIR, Biome.createCarver(new CaveWorldCarver(ProbabilityConfig::deserialize, 256) {
						{
							carvableBlocks = ImmutableSet.of(BurnedStoneBlock.block.getDefaultState().getBlock(),
