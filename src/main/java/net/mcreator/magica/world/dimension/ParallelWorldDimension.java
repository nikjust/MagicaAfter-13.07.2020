...
import net.mcreator.magica.item.ParallelWorldItem;
...
public class ParallelWorldDimension extends MagicaModElements.ModElement {
	@ObjectHolder("magica:parallel_world")
	public static final ModDimension dimension = null;
	@ObjectHolder("magica:parallel_world_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("parallel_world"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("magica:parallel_world")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("magica:parallel_world"), dimension, null, true);
		}
		type = DimensionType.byName(new ResourceLocation("magica:parallel_world"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("nether")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("magica:flovery_of_nether")),
				...
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("magica:entia_biome")),
				ForgeRegistries.BIOMES.getValue(new ResourceLocation("ocean")), ForgeRegistries.BIOMES.getValue(new ResourceLocation("desert")),
	...
	public void initElements() {
		...
		elements.items.add(() -> new ParallelWorldItem().setRegistryName("parallel_world"));
	}
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("parallel_world_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos p_180120_1_, Direction p_180120_2_) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos)) || !(this.world.getBlockState(blockpos.down())
							.getBlock() == Blocks.WITHER_SKELETON_WALL_SKULL.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = p_180120_1_.offset(p_180120_2_, i);
				return (this.world.getBlockState(framePos).getBlock() == Blocks.WITHER_SKELETON_WALL_SKULL.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == Blocks.WITHER_SKELETON_WALL_SKULL.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutableblockpos,
									flag
											? Blocks.WITHER_SKELETON_WALL_SKULL.getDefaultState().getBlock().getDefaultState()
											: Blocks.AIR.getDefaultState());
						}
					}
				}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos,
								Blocks.WITHER_SKELETON_WALL_SKULL.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
