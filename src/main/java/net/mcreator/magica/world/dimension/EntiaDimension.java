...
import net.mcreator.magica.item.EntiaItem;
...
public class EntiaDimension extends MagicaModElements.ModElement {
	@ObjectHolder("magica:entia")
	public static final ModDimension dimension = null;
	@ObjectHolder("magica:entia_portal")
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("entia"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("magica:entia")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("magica:entia"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("magica:entia"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("magica:entia_biome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new EntiaItem().setRegistryName("entia"));
	}
			...
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutableblockpos, Blocks.OAK_LOG.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
